            ...
            <form action="{{ route('tasks.update', $task->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                        ...
                        <option value="INPROGRESS" {{ old('status', $task->status) == 'INPROGRESS' ? 'selected' : '' }}>{{ __('In Progress') }}</option>
                ...
                <div class="mb-3">
                    ...
                    <select name="assignees[]" id="assignees" class="form-select @error('assignees') is-invalid @enderror" multiple>
                        @foreach ($teamMembers as $member)
                            <option value="{{ $member->id }}" {{ collect(old('assignees', $task->assignees->pluck('id')))->contains($member->id) ? 'selected' : '' }}>
                                {{ $member->name }}
                            </option>
                        @endforeach
                    </select>
