            ...
            <form action="{{ route('projects.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                        <option value="STARTED" {{ old('status') == 'STARTED' ? 'selected' : '' }}>{{ __('Started') }}</option>
                        <option value="ONHOLD" {{ old('status') == 'ONHOLD' ? 'selected' : '' }}>{{ __('On Hold') }}</option>
                ...
                <div class="mb-3">
                    ...
                    <select name="team_members[]" id="team_members" class="form-select @error('team_members') is-invalid @enderror" multiple>
                        @foreach ($teamMembers as $member)
                            <option value="{{ $member->id }}" {{ collect(old('team_members'))->contains($member->id) ? 'selected' : '' }}>
                                {{ $member->name }}
                            </option>
                        @endforeach
                    </select>
